...
export const AIOutcomePopup: React.FC<AIOutcomePopupProps> = ({ 
  ...
  return (
    <div className="fixed inset-0 bg-black/80 flex items-center justify-center z-50 p-4">
      <div className="retro-panel max-w-lg w-full p-6 rounded-lg relative">
        <button
          onClick={onClose}
          className="absolute top-4 right-4 text-retro-teal hover:text-retro-purple"
        >
          <X className="w-6 h-6" />
        </button>
        ...
        <div className="text-center space-y-6">
          {/* Title */}
          <h2 className="text-2xl font-bold text-retro-purple">OUTCOME</h2>

          {/* Outcome Text */}
          {showText && (
            <div className="bg-black/50 p-4 rounded border border-retro-teal/30">
              <p className="text-sm leading-relaxed typewriter">{outcome.outcome}</p>
            </div>
          )}
          ...
          {showMoney && moneyChange !== 0 && (
            <div className="flex items-center justify-center gap-4 py-4">
              <div className="flex items-center gap-2 text-gray-400">
                <span>${moneyBefore.toLocaleString()}</span>
              </div>
              ...
              <div className="flex items-center gap-2 text-retro-teal">
                <span className="text-xl font-bold">${moneyAfter.toLocaleString()}</span>
              </div>
            </div>
          )}
          ...
          {showBadge && outcome.badgeEarned && (
            <div className="bg-gradient-to-r from-retro-purple/20 to-retro-teal/20 p-4 rounded border border-retro-purple/50">
              <div className="flex items-center justify-center gap-2 mb-2">
                <Award className="w-6 h-6 text-retro-purple" />
                <span className="text-lg font-bold text-retro-purple">NEW BADGE!</span>
              </div>
              <p className="text-retro-teal">{outcome.badgeEarned}</p>
            </div>
          )}
          ...
          {outcome.consequence && (
            <div className="bg-orange-500/20 p-4 rounded border border-orange-500/50">
              <div className="flex items-center justify-center gap-2 mb-2">
                ...
                <span className="text-orange-400 font-bold">CONSEQUENCE</span>
              </div>
              <p className="text-orange-200 text-sm">{outcome.consequence}</p>
            </div>
          )}
          ...
          <button
            onClick={onClose}
            className="pixel-button text-retro-teal border-retro-teal hover:bg-retro-teal hover:text-black px-8 py-3"
          >
            Continue Adventure
          </button>
        </div>
